...
| `similarity_threshold` | Number | Minimum cosine similarity, between `0.0` and `1.0`, for a saved entry to be returned when SQL Lens searches its memory. Defaults to `0.7`. Lower the value if useful past answers are being missed; raise it if irrelevant past answers are surfacing. This value is the server-side default and can be overridden per call by the assistant when warranted. It is also the bar a memory must clear to be added automatically to the assistant's background context. |
| `context_similarity_threshold` | Number | Between `0.0` and `1.0`. Controls an optional, more permissive tier of *near-match* background context. Set it **below** `similarity_threshold` to have SQL Lens automatically include memories that score in between the two values — both saved question-and-answer pairs and free-form notes — as related context for the assistant on each question. Defaults to `0.7` (the same as `similarity_threshold`), which turns the near-match tier off and leaves behavior unchanged. The environment override is `SQLLENS_MEMORY__CONTEXT_SIMILARITY_THRESHOLD`. See [Managing memory](managing-memory.md#near-match-background-context). |
