...
| `read_only` | Boolean | When true (the default), only read-only statements are allowed: a `SELECT`, or (on MySQL) a structural `SHOW` schema-discovery command such as `SHOW TABLES` / `SHOW COLUMNS`. Generated SQL is parsed before execution, and anything else — including statements hidden inside subqueries or common table expressions, known unsafe or denial-of-service database functions, and information-disclosing `SHOW` variants such as `SHOW GRANTS` — is rejected. As a second line of defense, the database connection itself is also opened in read-only mode, so a write cannot reach your database even if it somehow slips past the parser. Keep this enabled in production. |
...
| `allow_admin_tools` | Boolean | Defaults to `false`. When set to `true`, SQL Lens exposes seven memory-administration tools to the connected assistant for inspecting and curating the saved memory: list, get, delete, clear, add, export and statistics. Leave this off unless you trust every client that can reach the server, since these tools read and change the saved memory. The tools that change memory (delete, clear and add) also refuse to run when the server requires no authentication, unless you set `auth.insecure` to acknowledge a closed network. The environment override is `SQLLENS_MEMORY__ALLOW_ADMIN_TOOLS`. See [Managing memory](managing-memory.md). |
...
| `show_details` | Boolean | Defaults to `false`. By default a `query_database` answer does not include the SQL the agent executed, because exposing that SQL to connected clients can reveal details about your database structure and query logic. Set to `true` to surface the executed SQL alongside each answer as a fenced ` ```sql ` block under an `**Executed SQL:**` header appended to the Markdown answer (visible in every MCP client). On hosts that read tool-result metadata the same SQL is also attached to the response under the `sqllens/query` key for programmatic / apps-aware consumers; the bundled result widget itself no longer renders the SQL as an in-iframe panel. Enabling the setting **also** attaches a structured **step-by-step agent trace** to the response metadata under the `sqllens/agent_trace` key — the tools the agent called, in order, with each call's arguments, status, duration, and (on failure) error, plus the run's terminal error — on both successful and failed answers, so a debugging client can see what the agent did and where a slow or failed run spent its time. Enable it only for trusted, debugging-oriented deployments. The environment override is `SQLLENS_AGENT__SHOW_DETAILS`. |
| `show_memory_details` | Boolean | Defaults to `false`. Controls whether each answer includes a short note about SQL Lens's memory lookup for that question. When set to `true`, an answer for which SQL Lens consulted its memory gains a one-line footer such as `Memory: 2 hits (top similarity 0.83)` on a match or `Memory: no matches` when nothing relevant was found. Only counts and similarity scores are ever shown — never the content of the remembered questions or answers. This footer is off by default to keep answers clean. The environment override is `SQLLENS_AGENT__SHOW_MEMORY_DETAILS`. **Note:** On hosts that support structured tool metadata, the same memory lookup summary (counts and scores only) is always available to the host regardless of this setting; the setting controls only the plain-text footer in the answer. |
