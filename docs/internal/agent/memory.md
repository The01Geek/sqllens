...
`args` is **optional** on `SaveQuestionToolArgsParams` (`default_factory=dict` in [src/sqllens/agent/tools/agent_memory.py](../../../src/sqllens/agent/tools/agent_memory.py)): an LLM-generated call that omits it validates and saves with `{}` instead of being rejected. This matters because not every tool the agent remembers carries meaningful args — the chart flow's `emit_chart` case sends only `{question, tool_name}`. If `args` were still required, that call would fail Pydantic validation at the tool registry's `model_validate` step (before `execute()` runs), surface as an error status card, and short-circuit `components_to_blocks`' chart-block construction — which is exactly how `visualize_data` was broken before issue #146. `question` and `tool_name` remain required.
...
| [src/sqllens/memory/io.py](../../../src/sqllens/memory/io.py) | Parse/serialize a bundle to and from JSON and CSV (bounded, whole-file). |
| [src/sqllens/memory/streaming.py](../../../src/sqllens/memory/streaming.py) | Memory-bounded streaming JSON reader for the CLI-only `--stream` path (issue #207). |
| [src/sqllens/memory/store.py](../../../src/sqllens/memory/store.py) | `MemoryStore` — thin adapter over the vendored `ChromaAgentMemory`. Includes `sql_pair_id` / `schema_doc_id` content-hash id helpers and the paginated `iter_paginated` enumerator. |
| [src/sqllens/memory/importer.py](../../../src/sqllens/memory/importer.py) | `import_bundle` (bounded) + `import_bundle_stream` (CLI streaming) — both write rows under a deterministic content-hash id, so re-import is an idempotent upsert. |
| [src/sqllens/memory/exporter.py](../../../src/sqllens/memory/exporter.py) | `export_bundle` (bounded) + `export_bundle_stream` (CLI streaming, atomic write) — enumerate a store and serialize it. |
...
Both caps surface a `BundleFormatError` ("bundle exceeds the 10485760-byte cap; split the bundle into smaller files." / "bundle 'sql_pairs.pairs' exceeds the 10000-item cap (got N); split the bundle."). The `import_memory` MCP tool re-raises that as a sanitized `RuntimeError("Invalid memory bundle: ...")` (`isError: true`); the `sqllens import-memory` CLI (default, bounded path) exits 1 with the same message on stderr.

**The CLI `--stream` path (issue #207) bypasses these whole-bundle caps**, while keeping the per-item caps (`QUESTION_MAX`, `SQL_MAX`, `CONTENT_MAX`, plus a per-value 1 MiB byte cap inside the streaming scanner) firing row by row. Memory-bounded streaming makes the DoS lever irrelevant for a local operator: the reader never holds more than one record (plus its read buffer) at a time, so an attacker-shaped 100 MB bundle cannot exhaust RAM. The MCP `import_memory` tool keeps the whole-bundle caps because remote clients are exactly the threat model the caps exist for.

Enforcing as a Pydantic `Field` constraint would propagate the cap to *every* construction — including `MemoryStore.iter_all` and `MemoryStore.iter_paginated` (which back `export_bundle` and `export_bundle_stream`) — so a healthy store legitimately holding more than `MAX_BUNDLE_ITEMS` rows would blow up at export. Putting the cap at the parse boundary instead leaves in-process readers/writers unrestricted while still refusing oversized external payloads on the way in.
...
`MemoryStore.add_sql_pair` (and its batch sibling `add_sql_pair_batch`) must write a pair in the **exact shape the agent writes at query time** or retrieval will never match it. The metadata is byte-identical to `ChromaAgentMemory.save_tool_usage`'s writes: `tool_name="run_sql"` (`RUN_SQL_TOOL_NAME` in [store.py](../../../src/sqllens/memory/store.py) — the default name of `RunSqlTool`), `args_json={"sql": ...}`, `success=True`, and `metadata_json={"source": "import"}`. `RUN_SQL_TOOL_NAME` is asserted against the live tool in the test suite (`tests/unit/test_run_sql_tool_name.py`) so a future rename of `RunSqlTool` can't silently break retrieval. Schema docs use the same wire shape as `save_text_memory` (`is_text_memory=True`, `content`, `timestamp`).

The import path no longer calls the vendored `save_tool_usage` / `save_text_memory` directly: those generate a fresh `uuid4` per write and would duplicate rows on every re-import. `MemoryStore` instead `upsert`s through the raw `_get_collection()` seam using a **deterministic content-hash id**: `sql_pair_id(question, sql) = sha256("sql_pair\0" + _norm(question) + "\0" + _norm(sql))` and `schema_doc_id(content) = sha256("schema_doc\0" + _norm(content))`, both defined in [store.py](../../../src/sqllens/memory/store.py). `_norm` is the same strip/collapse-whitespace/lowercase normalizer used historically for dedup. Two near-identical inputs (different casing, extra whitespace) hash to the same id and therefore upsert the same row — re-importing a curated bundle is idempotent, with no separate "skipped duplicate" count to surface.

`MemoryStore` constructs `ChromaAgentMemory` exactly as `build_agent` does (same `persist_dir` / `collection`), so the CLI and the live server operate on the same collection. `iter_all` / `iter_paginated` / `clear` reach the private `_get_collection()` seam directly: the vendored class exposes no public "enumerate everything" method (only `get_recent_*` with a limit), and its public `clear_memories` is async and row-by-row. The fallbacks are deliberately isolated to `store.py` (see its module docstring). Both enumerators only re-materialize the two kinds the bundle can represent (`run_sql` tool memories with a `sql` arg → SQL pairs; text memories → schema docs); any other live-agent tool memory, or a corrupt/oversized row, is skipped (one aggregate `WARNING`), never fatal.

**Wholesale failure is not tolerated by `iter_all`:** if the collection has at least `_WHOLESALE_MIN_ROWS` (5) rows and ≥`_WHOLESALE_SKIP_RATIO` (90%) of them fail to reconstruct (e.g. a chromadb/schema version skew making every `args_json` unparseable), `iter_all` raises `MemoryCorruptionError` instead of silently returning an empty bundle — otherwise a destroyed store would export as a "successful" empty backup. `iter_paginated` deliberately does **not** enforce this rule: the skip ratio is a single-snapshot statistic and does not translate cleanly across pages, so the streaming exporter surfaces the cross-page skip count as a non-fatal `warnings` entry instead.

`MemoryStore.last_skipped_rows` records the most recent non-fatal skip count; `export_bundle` returns an `ExportResult(text, warnings)` and `export_bundle_stream` returns a `StreamExportResult(sql_pairs_count, schema_docs_count, skipped_rows, warnings)` so the CLI/MCP layer surfaces empty-store, partial-skip, and CSV-drops-schema-docs losses instead of printing an unconditional green success. The MCP `import_memory` tool serializes concurrent calls behind an `asyncio.Lock` (single closure-bound `MemoryStore`) — not for dedup races (content-hash upsert is idempotent) but because two concurrent imports of the same logical pair would otherwise produce undefined "last write wins" ordering against the same row.

### Idempotent content-hash upsert (issue #207, replaces v1 exact-match dedup)

`import_bundle` writes every record under its content-hash id via `collection.upsert`. Re-importing the same logical pair (or one whose `(question, sql)` normalizes to the same string under `_norm`) targets the same row and overwrites it in place — there is no separate "skipped (duplicate)" count tracked on `ImportReport`, and the `store.iter_all()`-based seen-set baseline that used to seed dedup is gone. `clear=True` still wipes the collection before importing; `dry_run=True` still validates and reports but writes nothing **and skips the clear**; `batch_size` still bounds how many writes are issued before `await asyncio.sleep(0)` yields. A per-item write failure is recorded in `ImportReport.errors` and does not abort the rest of the batch.

There is no fuzzy/semantic dedup. Near-identical inputs that survive `_norm` (different punctuation, paraphrased questions over the same SQL, schema docs with different prose explaining the same column) still produce distinct rows.

### Streaming import (`--stream`, CLI only)

`import_bundle_stream` in [importer.py](../../../src/sqllens/memory/importer.py) is the CLI-only counterpart to `import_bundle`. It reads the JSON bundle one record at a time via the hand-rolled, escape-aware scanner in [streaming.py](../../../src/sqllens/memory/streaming.py) (stdlib only — no `ijson` dependency), validates each record against `SqlPair` / `SchemaDoc` (so per-item caps fire row by row), and upserts in batches of `batch_size` records through `MemoryStore.add_sql_pair_batch` / `add_schema_doc_batch`. Memory is bounded to one batch plus the read buffer regardless of total file size; a 100 MB bundle imports without loading the full document into RAM.

The streaming reader pauses buffer compaction during a single value scan so the slice indices captured by `read_value_raw` cannot shift underneath the caller (see `_Scanner._compaction_paused` in [streaming.py](../../../src/sqllens/memory/streaming.py)). A per-value 1 MiB byte cap (`_MAX_VALUE_BYTES`) bounds an attacker-shaped unterminated value that the per-item Pydantic caps would otherwise let grow without bound. The scanner's `{`, `[`, `"` framing is escape-aware: a SQL literal whose contents include a `}` does not fool the depth counter.

Failure model is documented on `StreamImportResult` and surfaced in the CLI:

- **Per-item validation failure** → recorded in `report.errors`, stream continues.
- **Batch upsert failure** → every item in the batch is recorded in `report.errors` and `report.saved` does NOT advance. Per CLAUDE.md "partial failure is failure" the CLI exits non-zero on any non-empty `report.errors`.
- **Systemic failure** (`MemoryError` / `OSError` / `SystemError`) → re-raised. Partial writes already in the store are not rolled back; the CLI surfaces a loud data-loss warning if `--clear` had run.
- **Malformed JSON / framing failure mid-stream** → sets `aborted=True` on the result. Counts reflect what landed before the failure point. CLI exits non-zero with the abort reason and the data-loss warning on `--clear`.
- **Likely format mismatch** (`likely_format_mismatch=True`): an input larger than `_LIKELY_FORMAT_MISMATCH_THRESHOLD` (64 bytes) that produced zero records is almost always a wrong-file / wrong-format mistake. The CLI exits non-zero with a loud warning. A genuinely empty bundle (e.g. `{}`) does not trigger this flag.
- **`--clear` + zero-records guard:** a `--clear` that wiped the store followed by a clean import that saved zero records (no errors, no abort, file too small to trip the format-mismatch heuristic) exits non-zero with a loud warning. The earlier `--clear` confirm dialog gates the wipe, so the operator did consent to the destruction; what they need now is a loud signal that nothing replaced what was wiped.

### Streaming export (`--stream`, CLI only)

`export_bundle_stream` in [exporter.py](../../../src/sqllens/memory/exporter.py) makes two passes through `MemoryStore.iter_paginated` (one filtered for `tool_name="run_sql"`, one for `is_text_memory=True`) and writes the bundle to disk one record at a time. The on-disk shape matches `export_bundle`'s JSON output exactly — both the streaming reader and the existing bounded `parse_json` can read it back.

The write is **atomic**: the exporter streams into a sibling `<path>.tmp` file and only `os.replace`s it over the destination after the close-out write succeeds. A mid-stream disk-full therefore cannot destroy the operator's previous backup at the same path. The temp file is best-effort cleaned up on any `BaseException` (so a `KeyboardInterrupt` during a long export still leaves the working tree tidy).
...
`import-memory` exit codes: `2` for a config-load error (consistent with `serve`/`validate`), `1` for a bad `--format`, an unreadable file, an invalid bundle, a store/import failure, any per-item import error in the report, a streaming-mode mid-file abort, a likely format mismatch, or the `--clear` + zero-records guard. `--clear` prompts for confirmation (`typer.confirm(..., abort=True)`) unless combined with `--dry-run`. If a `--clear` import fails *after* the store was constructed, the error message states the collection may now be empty/partial (the wipe already ran). Store/import errors carry the standard "embedding model downloads on first use (~80 MB); check persist_dir" hint. **The CLI commands work regardless of `allow_import`** — that flag only gates the MCP tool.

`--stream` (issue #207) is **JSON-only** on both commands (CSV bundles always fit the bounded path — the entire CSV body is one `csv.writer` call, so streaming it adds no headroom). On `import-memory` it switches to `import_bundle_stream`, lifting the whole-bundle DoS caps for local operators backing up large stores; on `export-memory` it switches to `export_bundle_stream`, paginating the collection and writing the bundle atomically via a sibling `.tmp` file + `os.replace`. Streaming export's cross-page skip count surfaces as a `Warning:` instead of a wholesale-corruption raise (see `iter_paginated` above). Streaming import cannot roll back — the CLI surfaces any failure as a non-zero exit and, when `--clear` had run, a loud data-loss warning.
...
The tool accepts JSON only (no CSV over MCP), runs `import_bundle` (the bounded path — the streaming path is CLI-only) with default `clear=False` / `dry_run=False`, and returns `ImportReport.to_markdown()` (a `saved` / `errors` table) **only when the import was fully clean**. The whole-bundle `MAX_BUNDLE_BYTES` / `MAX_BUNDLE_ITEMS` caps still apply on this path — the DoS guard is exactly what remote clients are gated by. Per the CLAUDE.md `isError` contract:

- A parse failure raises `RuntimeError("Invalid memory bundle: ...")`.
- A store/write failure raises a sanitized `RuntimeError` (logged via `logger.exception` / `logger.error`, never leaking the persist path or a raw traceback). The dedicated `MemoryCorruptionError` handler that distinguished a corrupt-baseline failure from a generic write failure was removed alongside the `iter_all`-based dedup baseline (issue #207) — content-hash upsert no longer reads the store before writing, so a corrupt baseline cannot bias an import. The admin tools' `export_memories` still distinguishes `MemoryCorruptionError` because `iter_all` is on its read path.
- **Any per-item error is a failure — partial failure is failure.** When `report.errors` is non-empty the tool raises, *even if some pairs saved and only others errored*. The guard is `if report.errors:`, so a run that saved some items and errored on the rest no longer reaches the client as `isError:false`. The client-facing message is counts-only — `"Memory import failed: N item(s) errored (X saved). A partial import is a failure; check the server logs."` — and the full per-item exception text (which can carry the on-disk persist path / driver internals) goes to the server log via `logger.error`, never to the client. This is pinned by `tests/unit/test_memory_mcp_tool.py::test_tool_signals_error_on_partial_failure`.
...
- **`add_memories` round-trip / partial failure.** Per-item validation failures (a malformed `SqlPair` / `SchemaDoc`, including a non-dict item) are collected into `errors` with the original input index rather than aborting the batch; valid items go through `import_bundle` (the same idempotent-upsert path the CLI default and `import_memory` use). `import_bundle`'s own per-item errors are mapped back to the caller's original indices. Returns `{saved_count, errors}` — the `duplicate_count` / `skipped_count` fields were dropped in issue #207 alongside the dedup baseline (content-hash upsert makes them meaningless). The server treats any non-empty `errors` as an `isError` result.
...
The MCP formatter ([src/sqllens/tools/_format.py](../../../src/sqllens/tools/_format.py)) reads that card into a `memory_info` payload, which the server attaches to `_meta["sqllens/memory_info"]` whenever a search completed (independent of any config flag). The answer body itself never carries an inline memory footer — the structured `_meta` channel is the single source of truth for the hit/miss signal. See [mcp-server/tools.md](../mcp-server/tools.md#memory-hitmiss-channel--metasqllensmemory_info) for the full channel mechanism.
