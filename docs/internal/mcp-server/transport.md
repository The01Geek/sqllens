...
Both failure-path `except Exception` blocks (startup `__aenter__` and shutdown `__aexit__`) are **deliberately scoped to `Exception`, never `BaseException` or a bare `except`**. `BaseException`-only subclasses — `asyncio.CancelledError`, `KeyboardInterrupt`, `SystemExit` — are allowed to propagate so structured-concurrency cancellation and interpreter teardown unwind the host cleanly instead of being swallowed and misreported as a `startup.failed`/`shutdown.failed` ack. Inline comments at both `except` sites record this. The contract is locked from both ends by `tests/unit/test_transport_http.py::test_lifespan_startup_baseexception_propagates_not_caught` and `::test_lifespan_shutdown_baseexception_propagates_not_caught` (each raises `KeyboardInterrupt` from the awaited CM method and asserts it escapes with no fabricated ack). The `_FakeSessionManager` test fake accepts `BaseException` for its `startup_exc`/`shutdown_exc` injection points so these signals can be exercised.
