...
- **First-query latency is eliminated (issue #116).** The warmup primes the *same* request-path `_AGENT_STATE` singleton via `prime_agent` (not a throwaway agent), so a successful warmup means the first real `query_database` *or* `visualize_data` call reuses the already-built agent and pays no ChromaDB / embedding-model cold start (both MCP tools share the singleton via [`tools/_agent.py`](../../../src/sqllens/tools/_agent.py); see [mcp-server/tools.md](./tools.md#shared-agent-singleton-tools_agentpy)).
...
`build_asgi_app(cfg)` wires this hook to a `_warmup` closure that calls `prime_agent(cfg)` (defined in [src/sqllens/tools/_agent.py](../../../src/sqllens/tools/_agent.py) and re-exported from [src/sqllens/tools/query_database.py](../../../src/sqllens/tools/query_database.py) so existing imports — including `transport/http.py`'s — still resolve; see [mcp-server/tools.md](./tools.md#eager-warmup-shares-the-request-path-singleton-issue-116)). `prime_agent` delegates to the same `get_agent` double-checked-lock singleton the request path serves, so the agent object graph is built **exactly once** and shared between boot and the first `query_database` *or* `visualize_data` call — *and then runs `_warm_memory` to force the otherwise-lazy ChromaDB open + ~80 MB embedding-model download* (`build_agent` alone only wires objects; see [mcp-server/tools.md](./tools.md#eager-warmup-shares-the-request-path-singleton-issue-116)). The full cold-start cost (DB connect, ChromaDB open, embedding-model download) is therefore paid at server boot, not on the first query. The `_warmup` closure binds `cfg` into the zero-arg `on_startup` signature; the closed-over `cfg` is the same `Config` object identity `get_agent`'s mismatch warning expects, so warmup does not false-trigger that warning (pinned by `tests/unit/test_transport_http.py::test_build_asgi_app_warmup_primes_singleton_with_closed_over_cfg`).
