...
- **Unknown / missing message types terminate the loop.** Earlier code logged a warning and fell through to the next `await receive()`, which on a misbehaving host turns into a silent busy-wait. The branch now logs `unknown lifespan message type: %r` (using `message.get("type")` so a malformed message with no `"type"` key falls through to the same branch instead of raising `KeyError`) and `return`s, ending the lifespan task cleanly. Covered by `tests/unit/test_transport_http.py::test_lifespan_unknown_message_type_terminates_loop` and `::test_lifespan_missing_message_type_terminates_loop`.
