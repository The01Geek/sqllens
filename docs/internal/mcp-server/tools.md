...
`components_to_blocks` captures the **last** such card's dict into `memory_info` (last-wins). `memory_info` is `None` on the error path and whenever no `memory_search` card was seen. From there the signal is surfaced on two independent rails:
