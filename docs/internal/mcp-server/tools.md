...
| `DATAFRAME` | Build the per-block table payload via `_build_table_payload` and emit `{"type": "table", ...payload...}` at its stream position. Every DataFrame becomes its own block — there is no "last DataFrame wins" collapse. Exception (#257): when the payload holds exactly one row, between one and `_SMALL_RESULT_MAX_COLUMNS` (4) columns, and nothing truncated, `_small_result_to_markdown` renders it as a single `**column:** value` line for one column, or a one-row Markdown table (header, `---` separator, value row) for two or more, with column names and values backslash-escaped (including `|`, so a value cannot split a cell) for Markdown syntax and CR and LF line breaks replaced with spaces, and the block is emitted as a public `{"type": "text", "text": ...}` block instead of a table. It is not filtered by the answer marker. Table-cell escaping on the table path is unchanged. The Markdown serializer (used by non-apps clients) renders each table block as a Markdown table and **caps the rendered rows at 500** (`_MAX_ROWS_RENDERED`) with a "Showing first N of M rows" footer; this rendering cap sits *above* `DatabaseConfig.max_rows` (default 10 000), which already stopped a larger DataFrame from being materialised. |
...
- `{"type": "text", "text": "<markdown>"}` — deliberate user-visible prose (see the TEXT row above), or a one-row query result of one to four columns with nothing truncated, rendered as an escaped `**column:** value` line or one-row Markdown table (see the DATAFRAME row above). The two are indistinguishable on the wire. No additional fields.
...
- **Text block** → a `<div class="block textblock">` holding the `text` field rendered as Markdown by the vendored **markdown-it** (`src/sqllens/ui/vendor/markdown-it.min.js`, inlined into the widget by `load_widget_html()` alongside ECharts and the ext-apps SDK; version/sha256 in `vendor/README`). `createMarkdownRenderer` (between the `md-renderer:start` / `md-renderer:end` markers) is the widget's XSS boundary because its output is assigned to `innerHTML`: `html: false` escapes raw HTML, markdown-it's default `validateLink` leaves `javascript:` / `vbscript:` / `file:` / `data:` targets as literal text, image syntax is disabled so the widget never fetches a remote URL (`![a](u)` degrades to `!` + a plain link), `breaks: true` keeps single newlines as `<br>`, links get `target="_blank" rel="noopener noreferrer"`, and GFM tables are wrapped in the `.wrap` scroll container. If the `markdownit` global is missing the block degrades to raw text instead of breaking the widget. Behaviour is pinned by `tests/unit/test_ui_widget_markdown.py`, which runs the extracted factory under Node with the vendored bundle (skipped when `node` is not on `PATH`).
