...
| Agent emitted an error status card | `AgentRunError("SQL execution error: " + <card description>)` (a `RuntimeError` subclass) → `isError: true`. Agent-authored detail passed through (categorized, not sanitized); logged server-side too. When `agent.show_details` is on the raised error carries the step-by-step trace, which `server.py` attaches to the `isError` result's `_meta["sqllens/agent_trace"]`; with it off the trace is `None` and the failure is formatted exactly as before. See [Agent trace channel](#agent-trace-channel--agentshow_details). |
