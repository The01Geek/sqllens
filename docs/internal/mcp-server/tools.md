...
- **Text block** → a `<div class="block textblock">` holding the `text` field rendered as Markdown by the in-file `renderMarkdown` (between the `md-renderer:start` / `md-renderer:end` markers). It supports ATX headings, paragraphs (single newlines kept as `<br>`), bold/italic (underscore emphasis needs non-word neighbours so `snake_case` identifiers stay literal), inline and fenced code, links (only `http:`/`https:`/`mailto:` become anchors; any other scheme stays plain text), nested bullet/numbered lists, blockquotes, horizontal rules, and GFM pipe tables (static — no sort/filter). Every node is built with `createElement`/`createTextNode`, never `innerHTML`, so agent output cannot inject markup. Behaviour is pinned by `tests/unit/test_ui_widget_markdown.py`, which extracts the renderer and runs it under Node against a small DOM shim (skipped when `node` is not on `PATH`).
