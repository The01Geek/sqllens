...
`build_server` in [src/sqllens/server.py](../../../src/sqllens/server.py) always registers the two core tools — plus a single `ui://` resource backing the `query_database` widget (which renders the agent's ordered `blocks` array: text, table, and chart blocks in stream order) — on a fresh `FastMCP("sqllens")` instance per call, and conditionally a third:
...
def build_server(cfg: Config) -> FastMCP:
    ...
    async def query_database(
        ...
        extra_meta: dict = {}
        if blocks:
            extra_meta["sqllens/blocks"] = blocks
        if query_info:
...
3. **Stream collapse, threading `conversation_id`.** The agent yields an async stream of `UiComponent` objects (text snippets, dataframes, chart components, status cards). MCP tools must return a single string, so we collect the stream into a list and pass it to `components_to_blocks` (the success path; `components_to_markdown` is a thin view over it that drops every structured payload — see "The `_format.components_to_blocks` collapse rule"). The `conversation_id` argument is forwarded into `agent.send_message(..., conversation_id=...)` so a follow-up turn loads the prior `Conversation`'s message history and the agent can answer its own clarifying question (see "Multi-turn conversations (`conversation_id`)" below).
...
## `_format.components_to_blocks` — the collapse rule

`components_to_blocks` in [src/sqllens/tools/_format.py](../../../src/sqllens/tools/_format.py) is the only place that knows the shape of the agent's output stream. It does the collapse in a single ordered pass and returns `(markdown, is_error, blocks, query_info, memory_info)` — where `blocks` is the ordered list of typed dicts that becomes `_meta["sqllens/blocks"]`. The narrower `components_to_markdown` view drops every structured payload for non-apps callers (see "The MCP App interactive result widget"). The per-component rules:

| Component type | What we do with it |
|---|---|
| `TEXT` (answer-marked) | A TEXT component whose `data[IS_ANSWER_MARKER_KEY] is True` is a **deliberate prose block**: the agent's terminal answer, the iteration-limit warning, or any `emit_text` output (see "Text composition: the `EmitTextTool` seam" below). Each marked TEXT becomes a `{"type": "text", "text": ...}` block at its stream position. |
| `TEXT` (unmarked) | Intermediate reasoning chatter the LLM emits during a tool call (gated on `UI_FEATURE_SHOW_TOOL_INVOCATION_MESSAGE_IN_CHAT`). Normally **excluded** from blocks. Backwards-compat fallback only: when *no* answer-marked TEXT was seen in the whole stream (e.g. a test fixture using bare `RichTextComponent`, or an abnormal termination where the agent never reached its terminal-answer yield), the *last* non-empty unmarked TEXT is included as a text block. An `INFO`-level server-side log fires on that fallback path so an operator can spot a marker-emission regression. |
| `DATAFRAME` | Build the per-block table payload via `_build_table_payload` and emit `{"type": "table", ...payload...}` at its stream position. Every DataFrame becomes its own block — there is no "last DataFrame wins" collapse. The Markdown serializer (used by non-apps clients) renders each table block as a Markdown table and **caps the rendered rows at 500** (`_MAX_ROWS_RENDERED`) with a "Showing first N of M rows" footer; this rendering cap sits *above* `DatabaseConfig.max_rows` (default 10 000), which already stopped a larger DataFrame from being materialised. |
| `CHART` | Build the per-block chart payload via `_build_chart_payload` and emit `{"type": "chart", ...payload...}` at its stream position. `emit_chart` may be called **more than once per request** (#194) — each call produces a separate chart block in stream order. The Markdown serializer renders each chart block as a `_[chart: <title>]_` placeholder; apps-aware hosts render the real ECharts chart from the `_meta["sqllens/blocks"]` channel. |
| `STATUS_CARD` with `status == 'error'` | Set `error_message` (last-wins). This marks the turn an error **unless a later *data* `run_sql` success card supersedes it** — see [Agent self-correction supersedes an earlier error](#agent-self-correction-supersedes-an-earlier-error) below. A still-set `error_message` at the end of the pass returns `(message, is_error=True, [], None, None)` and the caller raises `RuntimeError`. |
| `STATUS_CARD` with `status == 'success'` carrying a *data* `metadata["sql"]` | If an earlier `error_message` is set, **clear it** — the agent re-issued a corrected query after a failure and the recovered answer must reach the caller. Scoped two ways so it cannot mask a real failure: the card must carry `metadata["sql"]` (a memory-search / chart success never clears), and the SQL must **not** be a schema-introspection read (`is_introspection_query` — see [database-connectors/read-only-safety.md](../database-connectors/read-only-safety.md#schema-introspection-classifier-is_introspection_query)). The clear logs `INFO` server-side. See [Agent self-correction supersedes an earlier error](#agent-self-correction-supersedes-an-earlier-error). |
| `STATUS_CARD` carrying `metadata["sql"]` | Capture the SQL into `last_sql` (last-wins; the `run_sql` card streams twice — running → completed — with identical metadata, so dedup is idempotent). Drives the `query_info` channel — see [Executed SQL channel](#executed-sql-channel--agentshow_details). Only emitted when `agent.show_details` unlocked `UI_FEATURE_SHOW_TOOL_ARGUMENTS` for the static user group. The walker additionally tracks a per-`run_sql` row count (reset on the `running` card or on any SQL-text change against the previous card) so `query_info.row_count` is attributed to the *last* `run_sql` even when the agent ran multiple `run_sql` calls in one turn. |
| `STATUS_CARD` carrying `metadata["memory_search"]` | Capture the dict into `last_memory` (last-wins). This is the `Memory Search` card the `search_saved_correct_tool_uses` tool emits on **both** its hit and miss success paths — independent of `agent.show_details`. Drives the `memory_info` channel — see [Memory hit/miss channel](#memory-hitmiss-channel--metasqllensmemory_info). |
| Everything else | Ignored. |

### The ordered `sqllens/blocks` wire contract
...
The previous flat `_meta["sqllens/chart"]` and `_meta["sqllens/table"]` channels were retired (#194); the single ordered `_meta["sqllens/blocks"]` array replaces them. The widget renders the blocks in stream order with **no chart-vs-table precedence and no "chart wins" rule** — the order is the agent's. The other `_meta` channels (`sqllens/query`, `sqllens/memory_info`, `sqllens/agent_trace`, `sqllens/conversation`) are unaffected by this cutover and continue to ride alongside the blocks channel.

### Size budgets

- **Per-block ceiling.** Each table block is bounded by `_MAX_TABLE_PAYLOAD_BYTES = 130 * 1024` and each chart block by `_MAX_CHART_PAYLOAD_BYTES` (aliased to the table budget so the two cannot drift apart) — enforced by the existing binary-search prefix trim inside `_compute_table_payload` / `_compute_chart_payload`. The kept row prefix is reported in `row_count`; the dropped tail count rides on `truncated`.
- **Whole-array ceiling.** `_MAX_BLOCKS_TOTAL_BYTES = 512 * 1024` caps the *serialized size of the whole `blocks` array* so a long stream of in-budget blocks still cannot blow the iframe `_meta` ceiling. `_trim_blocks_to_budget` drops trailing blocks one at a time, re-measures with an appended truncation notice, and returns the prefix that fits **plus** an explicit `{"type": "text", "text": "_Response truncated: N trailing block(s) dropped to fit the rendering size budget._"}` text block. A `logger.warning` fires whenever any trimming happens (CLAUDE.md: never silently drop). The degenerate case where even a single block + notice busts the budget — a misconfiguration, not a normal state — returns a notice-only single-block list and logs `ERROR` rather than rendering as a silent empty / `"(no answer)"` to the user.

The markdown answer is `_serialize_blocks_to_markdown(blocks)` over the (possibly trimmed) blocks: tables become Markdown tables, text blocks pass through verbatim, charts collapse to a `_[chart: <title|chart_type>]_` placeholder (or `_[chart unavailable]_` when neither is present). Element order follows the agent's stream rather than the previous collapse's tables-then-text convention — a `[text, table]` stream now renders text before the table. An unknown discriminator emits a visible `_[unsupported block type: ``<type>``]_` placeholder and a `WARNING` log (symmetric with the widget's unknown-block handling) rather than silently dropping the block.

### Text composition: the `EmitTextTool` seam

`EmitTextTool` ([src/sqllens/agent/tools/emit_text.py](../../../src/sqllens/agent/tools/emit_text.py)) is the first-party agent tool the agent calls to author deliberate prose blocks at chosen positions in the response stream. It mirrors `EmitChartTool`: a single-field Pydantic args model (`text: str`, rejected as empty/whitespace-only at validation), and an `execute` body that emits a `RichTextComponent` whose `data` carries the answer marker. The MCP-layer block builder reads that marker (see the TEXT row above) and lifts the prose into a `{"type": "text", ...}` block at the call's stream position.

The marker convention is centralized in [src/sqllens/agent/markers.py](../../../src/sqllens/agent/markers.py): the key is `IS_ANSWER_MARKER_KEY = "is_answer"` and a tiny helper `answer_marker_data()` builds the `{"is_answer": True}` dict the producers stamp into `RichTextComponent.data`. Three producers set the marker today and only those three reach the rendered answer:

1. The agent's **terminal answer** at end-of-turn (`agent/core/agent/agent.py`, the LLM-loop completion yield).
2. The agent's **iteration-limit warning** when `max_tool_iterations` is hit (same module, the bail-out yield).
3. Every `EmitTextTool` execution.

Intermediate reasoning TEXT the agent emits *during* a tool call (gated on `UI_FEATURE_SHOW_TOOL_INVOCATION_MESSAGE_IN_CHAT`) is deliberately not marked, so it stays out of the rendered answer — that is the discriminator. The check is strict-identity (`is True`) so a producer drift that stuffs a non-`bool` value under the marker key (e.g. the string `"yes"`) does not silently flip the discrimination.

The system prompt's `ANSWER COMPOSITION (emit_text)` block (added by `DefaultSystemPromptBuilder` when `emit_text` is registered) tells the LLM that *only* `emit_text` puts deliberate prose in the rendered answer, that it can be called more than once per request, and gives a few-shot interleaved example (`search_saved_correct_tool_uses → run_sql → emit_chart → emit_text → run_sql → emit_text`). The same block also relaxes the previous `emit_chart` "EXACTLY ONCE / never more than once per request" restriction: `emit_chart` may now be called any number of times in a single request when the answer warrants multiple distinct charts (each call appends a separate chart block in stream order).

### Agent self-correction supersedes an earlier error

The error signal is **terminal-status / last-wins**, not first-wins. A single turn can contain several `run_sql` cards: the agent commonly issues a query that fails ("Unknown column", a bad table name), confirms the real name with a catalog lookup, and re-issues a corrected query that succeeds — especially on a fresh database, after a wiped memory, or against non-obvious column names. Before this change, the first error card marked the whole turn `isError` and the recovered answer never reached the caller, who saw a spurious `SQL execution error` for a question the agent had actually answered.

`components_to_blocks` now lets a later **data** `run_sql` success card clear an earlier `error_message`. The clear is scoped two ways so it can never mask a genuine failure:

1. **`run_sql` only.** The clearing card must carry a truthy `metadata["sql"]`. A successful `Memory Search` card (which runs before the query on most turns) or a successful `emit_chart` card never clears a query failure.
2. **Not schema introspection.** The SQL must not be a schema-introspection read, tested by `is_introspection_query` (see [database-connectors/read-only-safety.md](../database-connectors/read-only-safety.md#schema-introspection-classifier-is_introspection_query)). The system prompt has the agent run an `information_schema` / `pg_catalog` / `SHOW` lookup to confirm a column before retrying; that lookup succeeding is a *step toward* an answer, not the answer. Counting it would let "data query failed → introspect → give up" report a silent success.

A genuinely failing **final** data `run_sql` (with no later data-`run_sql` success) keeps `error_message` set and still surfaces as `isError: true`. When a clear does happen, `components_to_blocks` logs at `INFO` (`"run_sql error superseded by a later successful run_sql in the same turn; surfacing the recovered answer"`) so an operator can see self-correction in the logs. Pinned by `tests/unit/test_format.py`.

Output ordering: every block (text / table / chart) is emitted at its stream position — there is no "tables first, then text" rearrangement. If the blocks list is empty, fall back to `render_interactive(components)` — the agent's interactive/follow-up affordances rendered as plain Markdown (see "Surfacing interactive affordances" below) — and only when *that* is also empty return `"(no answer)"` rather than the empty string (MCP clients render empty results badly). To enable this second pass over the stream, `components_to_blocks` `list()`-materializes its `components` argument up front (the public signature still accepts any `Iterable`, including generators).

## Surfacing interactive affordances (`render_interactive`)

When a turn produces **no** blocks (no DataFrame, no chart, no answer-marked TEXT), the agent's only output may be an *interactive* affordance — a clarifying question it expressed as a UI component rather than as text. Without handling, the tool would return the useless `"(no answer)"`. `render_interactive` in [src/sqllens/tools/_format.py](../../../src/sqllens/tools/_format.py) renders those affordances as plain Markdown so the calling model receives the question and can answer it on the next turn. The output is independent of the MCP Apps widget channel, so non-apps clients get the question too. It is invoked **only** on the no-blocks fallback path (above), so a normal answer's trailing finalization components are never surfaced.
...
On **apps-aware hosts** (Claude Desktop, claude.ai), `query_database` additionally carries an interactive widget that renders the agent's ordered `blocks` array — one container per block (interactive ECharts chart, sortable/filterable/paginated/CSV-exportable data grid, or Markdown prose) in stream order. Every other host keeps receiving the byte-identical Markdown serialization above — the [MCP Apps spec](https://modelcontextprotocol.io) (`2026-01-26`) makes this degradation transparent.

One tool, one widget, one resource. The agent decides what to emit (it can call `run_sql`, `emit_chart`, and `emit_text` in any order, any number of times — see "The ordered `sqllens/blocks` wire contract" and "Text composition: the `EmitTextTool` seam" above); the widget just iterates the blocks and draws each one. There is **no chart-vs-table precedence** — the order is whatever the agent produced.
...
- `query_database_impl_with_widgets` is the single agent path; the legacy `query_database_impl` (drops every structured payload) is now a thin wrapper over it. Same agent path, same three error categories. On success it returns `(markdown, blocks, query_info, memory_info, agent_trace)` where `blocks` is the ordered list of typed dicts (`{"type": "text|table|chart", ...}` — see "The ordered `sqllens/blocks` wire contract" above; empty `[]` on the error path and on a turn that produced no structured artifacts), `query_info` carries the executed SQL when `agent.show_details` is on (see [Executed SQL channel](#executed-sql-channel--agentshow_details) below), `memory_info` carries the aggregate memory hit/miss signal when a memory search completed this turn (see [Memory hit/miss channel](#memory-hitmiss-channel--metasqllensmemory_info) below), and `agent_trace` carries the structured step-by-step loop trace when `agent.show_details` is on (`None` otherwise — see [Agent trace channel](#agent-trace-channel--agentshow_details) below). `_format.components_to_blocks` builds each table block from its own `DATAFRAME` component and each chart block from its own `CHART` component, enforcing the **130 KB per-block serialized-size budget** (`json.dumps(payload, separators=(",", ":"))`) by binary-searching a row prefix that fits, reporting the dropped tail in `truncated`; if even the header-only / data-stripped form is over budget that block is dropped silently from `blocks`. The whole array is then bounded by the **512 KB overall ceiling** (`_MAX_BLOCKS_TOTAL_BYTES`) via `_trim_blocks_to_budget`, which appends an explicit truncation-notice text block and logs `WARNING` (see "Size budgets" above). Payload construction is best-effort: any exception degrades that one block to "skipped" (Markdown still served, the rest of the blocks still ride) rather than escaping the sanitized error taxonomy.
- **Typed (numeric) client-side sort.** In each table block the widget right-aligns and numerically sorts a column only when `column_types[col] == "number"`. The vendored DataFrame producers never populate `column_types` (`DataFrameComponent.from_records` hard-codes `{}`), so `_format._compute_table_payload` **infers** the type server-side: a column is typed `"number"` when every non-empty coerced cell parses as a finite float (SQL `NULL`/empty cells are skipped; `inf`/`NaN` disqualify the column). Any explicit producer-supplied `column_types` overrides the inferred value (e.g. a zero-padded ID column the agent typed `"string"` stays string-sorted). A non-mapping `column_types` from a producer degrades to inference-only and never fails the payload. Without this inference, numeric columns would sort lexicographically (`1, 10, 100, 2`).
- The tool body returns a `CallToolResult` carrying the Markdown as text content **and** a `_meta` dict assembled from up to four keys: `"sqllens/blocks"` when `blocks` is non-empty, `"sqllens/query"` when `query_info` is truthy, `"sqllens/memory_info"` when `memory_info` is truthy, and `"sqllens/agent_trace"` when `agent_trace is not None`. Only `"sqllens/blocks"` is widget-consumed; the other three (`"sqllens/query"`, `"sqllens/memory_info"`, `"sqllens/agent_trace"`) are non-widget observability channels that ride on the `CallToolResult` for apps-aware / programmatic consumers — the result widget does not read them. See [Executed SQL channel](#executed-sql-channel--agentshow_details), [Memory hit/miss channel](#memory-hitmiss-channel--metasqllensmemory_info), and [Agent trace channel](#agent-trace-channel--agentshow_details) for each channel's shape. When all four keys are absent the tool returns the plain Markdown string (degrades cleanly for non-apps clients). The widget reads `"sqllens/blocks"` via the apps `ontoolresult` channel and **renders each block in stream order** — there is no chart-wins precedence and no "both attached, only chart drawn" deduplication. The executed SQL is surfaced to the user via the Markdown rail described in the Executed SQL channel section below; the `_meta["sqllens/query"]` channel itself stays attached for programmatic consumers but is not painted by the widget. `structured_output=False` is set on the tool so FastMCP does not derive an `outputSchema` that would reject the deliberately-absent `structuredContent`.

### Per-block widget rendering model

`renderBlocks(result)` in [src/sqllens/ui/query_results.html](../../../src/sqllens/ui/query_results.html) iterates `result._meta["sqllens/blocks"]` and creates one DOM container per block, each isolated from the rest:

- **Text block** → a `<div class="block textblock">` with `white-space: pre-wrap`. The raw `text` field is rendered as-is — a full Markdown parser would inflate the bundle for marginal gain; the host's plain-text fallback uses the same content.
- **Table block** → a `<div class="block tableblock">` whose **per-block state** (`{columns, rows, types, rowCount, truncated, sortCol, sortDir, filter, page}`) lives on the container's closure, not on widget-global state. The filter input, sort indicators, paginator, and "showing N of M" meta span belong to that one container. Sorting block 1 does not touch block 2's order; paging block 2 does not reset block 1's page; typing in block 3's filter only filters block 3's rows. Each table block also carries its own CSV-export pipeline (`ui/download-file` → `clipboard.writeText` → `execCommand('copy')` → inline textarea overlay), exporting only its own rows.
- **Chart block** → a `<div class="block charthost">` with a definite-height `<div class="chart">` child. Each chart block has its own ECharts instance and `ResizeObserver` tracked on a widget-level `chartRegistry: [{el, instance, observer, spec}, ...]` so theme swaps and tear-down work uniformly across all chart containers without intertwining. `renderChartBlock` only *appends* the entry; the real `echarts.init` happens in a finalization pass at the end of `renderBlocks` so every chart initializes against an already-in-DOM, properly-sized container (an ECharts instance created on a 0×0 not-yet-mounted node paints broken on first render).
- **Unknown discriminator** → a visible `Unsupported block type: <type>` placeholder is appended and a `console.warn` fires (symmetric with the Python serializer's `_[unsupported block type: …]_` placeholder). Silent drops are forbidden.

`teardownChartRegistry()` disposes every prior ECharts instance and disconnects every prior `ResizeObserver` before each `renderBlocks` call so re-rendering on a fresh `ontoolresult` event leaves no orphans. Theme changes (`applyHostContext`) re-iterate the registry and `rebuildChartInto(entry)` each chart in place (ECharts cannot swap theme on a live instance, so dispose + re-init is required); the per-block container is reused so stream order is preserved.
...
### Chart blocks: the `EmitChartTool` seam

Charts are produced by the agent, not by a second tool. The `EmitChartTool` agent-side seam (first-party, not lifted — see [src/sqllens/agent/tools/emit_chart.py](../../../src/sqllens/agent/tools/emit_chart.py)) is registered alongside `RunSqlTool` by `build_agent` (see [agent/factory.md](../agent/factory.md)). The agent runs `run_sql` first to get the aggregated rows, then calls `emit_chart` with those rows when the result is chart-shaped; `EmitChartTool` does **not** touch SQL — it only validates the renderer-agnostic DSL and emits a `ChartComponent`. The system prompt's `EMIT_CHART USAGE` block (added when `emit_chart` is registered) pins this workflow and **explicitly allows multiple `emit_chart` calls in a single request** (#194) when the answer warrants multiple distinct charts (e.g. one per metric, one per comparison axis); each call appends a separate `{"type": "chart", ...}` block to the response stream in call order.
...
- **`_meta` rail (apps-aware hosts / programmatic consumers).** `server.py` sets `_meta["sqllens/query"] = query_info` on the `CallToolResult` — a sibling channel to `_meta["sqllens/blocks"]`. The channel stays attached so apps-aware hosts and any client reading the result programmatically (debuggers, audit pipelines, observability tooling) can still pull the executed SQL off the `CallToolResult`. The widget at [src/sqllens/ui/query_results.html](../../../src/sqllens/ui/query_results.html) does **not** render this channel — `renderBlocks(result)` no longer reads `_meta["sqllens/query"]` and the prior collapsible `<details class="sql">` panel was removed in #202. The SQL still reaches the user via the Markdown rail above.

`query_info` is the dict `{"sql": str, "query_type": str, "row_count"?: int}` built by `_format._query_info_from_sql`:

- `sql` — the executed SQL string, captured verbatim from the `run_sql` STATUS_CARD's `metadata["sql"]`. The card streams twice (running → completed) with identical metadata, so the last-wins capture in `components_to_blocks` de-dupes idempotently. When the agent issues multiple `run_sql` calls in one turn (e.g. the introspect-then-retry path or a true multi-result answer), the last `run_sql`'s SQL wins.
...
- `row_count` — present whenever a DATAFRAME component followed the **last** `run_sql` card; absent when none did (no rows on stdout-only output, or a tool error before any DATAFRAME yield, **or** when the last `run_sql` returned zero rows even though an earlier `run_sql` in the same turn had rows — the walker resets the per-`run_sql` row tracker on the `running` card or on any SQL-text change, so the count is correctly attributed to the SQL that produced it rather than to an earlier table block in `blocks`). When present it is the **true** result size, not the rendered subset: if the per-block payload was built successfully, it is `payload["row_count"] + payload["truncated"]` (so the figure reflects the full set even when the 130 KB serialized-payload budget dropped tail rows); if the payload computer rejected the result (header-only over budget, or a caught construction exception), the count is recovered from the raw DataFrame component itself (`len(rich.rows)`) so an over-budget result does not silently report `row_count: 0`. The `.get(..., 0)` on the payload keeps a partial future shape from raising past the sanitized error taxonomy.

The "no SQL channel" branches are deliberately distinguishable:

- `agent.show_details = False`: the factory leaves `UI_FEATURE_SHOW_TOOL_ARGUMENTS` admin-gated, so the static user never receives the `run_sql` STATUS_CARD; `last_sql` in `components_to_blocks` stays `None` and `query_info` is `None`. Output is byte-for-byte the pre-feature behavior (no Markdown block, no `_meta["sqllens/query"]`).
...
- `agent.show_details = True` and the agent ran SQL but execution failed: the completed `run_sql` card carries `status="error"` (the upstream agent maps `ToolResult(success=False)` → `set_status("error", …)`), which fires the `error_message` short-circuit in `components_to_blocks` *before* `query_info` is built. The tool surfaces the sanitized error message through the [error contract](#the-errorsuccess-contract) below — the `sqllens/query` channel is never populated on the error path. (The `sqllens/agent_trace` channel **is** attached on this error path when `show_details` is on — see [Agent trace channel](#agent-trace-channel--agentshow_details); it is the one *observability/widget* channel — blocks/query/memory — that survives a failure. The conversation channel also rides the error result, via `_trace_error_result`.)
...
## Agent trace channel — `agent.show_details`

The same `agent.show_details` flag that unlocks the executed-SQL channel additionally emits a **structured step-by-step trace of the agent loop** under `_meta["sqllens/agent_trace"]` (issue #180). It is gated on the same flag deliberately: `show_details` already admits schema/SQL leakage to "trusted, debugging-oriented deployments", and a per-step trace falls under the same threat model — so this adds no new security knob. With the flag **off** (the default) no trace is built and the channel is absent — behaviour is byte-for-byte unchanged.

The trace is the answer to "what did the agent actually do, and where did a slow or failed run spend its time?" — the question the executed-SQL card alone could not answer. It is assembled by `build_agent_trace` in [src/sqllens/tools/_format.py](../../../src/sqllens/tools/_format.py) in a **single forward pass over the same buffered component stream** — the agent is never re-run or re-instrumented (issue #180 scope note). Shape:

```jsonc
{
  "iterations": 2,
  "max_iterations": 20,
  "total_duration_ms": 298412,
  "steps": [
    { "index": 0, "tool": "search_saved_correct_tool_uses",
      "arguments": { "question": "..." }, "status": "ok", "duration_ms": 412 },
    { "index": 1, "tool": "run_sql",
      "arguments": { "sql": "SELECT ..." }, "status": "error",
      "duration_ms": 287000, "error": "timeout after 240s" }
  ],
  "terminal_error": "tool 'run_sql' failed: timeout after 240s"
}
```

How each field is derived from the stream:
...
**Delivery on both success and failure.** Unlike every other `_meta` channel, the trace is attached on the **error path too**. This is essential: the vendored agent maps a failed tool to a `status="error"` card, which drives `components_to_blocks` to flag `is_error` and the tool to raise — so a tool failure, a DB timeout, and a top-level LLM/infra error all leave via the error path, and those are precisely the runs an operator most needs the trace for (only the max-iteration stop ends on the success path). The mechanism:

- The impl raises `AgentRunError` (a `RuntimeError` subclass) instead of a bare `RuntimeError` on the agent-error path, carrying the trace on `.agent_trace` (`None` when `show_details` is off). Because it subclasses `RuntimeError`, the message and every `except RuntimeError` caller are unchanged.
- `server.py` catches `AgentRunError`: when `.agent_trace` is not `None` it returns a `CallToolResult(isError=True, …)` with `_meta` carrying the trace (and the conversation id), and the text mirrors FastMCP's own `Error executing tool query_database: <message>` format so the client-facing message is byte-for-byte identical to the details-off path. When `.agent_trace` is `None` (details off) it re-raises so FastMCP formats the failure exactly as before, no `_meta` — a details-off deployment stays unchanged.

Pinned by `tests/unit/test_format.py` (`build_agent_trace` step pairing, terminal-error precedence, max-iteration, incomplete-step, empty-stream cases), `tests/unit/test_query_database.py` (trace on/off and `AgentRunError`-with-trace on the failure path), and `tests/unit/test_server.py` (`_meta["sqllens/agent_trace"]` on both the success and the `isError` result, and the re-raise when no trace).

## Memory hit/miss channel — `_meta["sqllens/memory_info"]`

The agent decides on its own when to consult vector memory (see [agent/memory.md](../agent/memory.md)). When it calls `search_saved_correct_tool_uses` (`SearchSavedCorrectToolUsesTool` in [src/sqllens/agent/tools/agent_memory.py](../../../src/sqllens/agent/tools/agent_memory.py)), that tool surfaces an **aggregate** hit/miss signal to the MCP layer. Only the fact of a hit/miss and counts/scores are exposed — **never the matched memory contents** (the recalled questions or args stay server-side; issue #168 deliberately scoped them out).
...
`components_to_blocks` captures the **last** such card's dict into `memory_info` (last-wins). `memory_info` is `None` on the error path and whenever no `memory_search` card was seen. The signal then rides a single rail:

- **`_meta` rail (always-on).** `server.py` sets `_meta["sqllens/memory_info"] = memory_info` on the `CallToolResult` whenever `memory_info` is truthy — i.e. whenever a memory search **completed** (a hit or a miss). This rail is **independent of `agent.show_details`** and gated by no flag; the structured signal is carried whenever a search completes. It is an observability channel, not a widget channel — the result widget does not read it. Apps-aware hosts read it directly off the `CallToolResult` and render their own hit/miss panel.

The answer body itself never carries an inline memory footer: the structured `_meta` channel is the single source of truth, so plain-text clients see the answer unadorned.
...
Pinned by `tests/unit/test_search_memory_threshold.py` (the tool emits the card with the right `memory_search` dict on hit, miss, and the error path), `tests/unit/test_format.py` (`components_to_blocks` extracts `memory_info`), `tests/unit/test_query_database.py` (`memory_info` returned on both hit and miss, no footer on the body), and `tests/unit/test_server.py` (`_meta["sqllens/memory_info"]` assembly).
...
- **Success result.** Returns `ImportReport.to_markdown()` — a `| metric | count |` table of `saved` / `errors` — **only when the import was fully clean** (`report.errors` empty). A normal (`isError: false`) result is therefore returned only when every item that the bundle promised was saved (or idempotently upserted over an existing row), with no per-item failures. The `skipped (duplicate)` row was dropped in issue #207: rows are now upserted under a deterministic content-hash id, so re-importing the same logical pair overwrites in place rather than being counted as a separate "skip".
- **Error contract.** Per CLAUDE.md's `isError` rule:
  ...
  - A store/write failure (Chroma, embedding download, disk) raises a sanitized `RuntimeError("Memory import failed while writing to the store; ... Check the server logs.")`, with the full detail logged server-side so the persist path is not leaked to the client. The dedicated `MemoryCorruptionError` arm that previously distinguished a corrupt-baseline failure was removed alongside the `iter_all`-based dedup baseline (issue #207) — content-hash upsert no longer reads the store before writing, so a corrupt baseline cannot bias an import. `export_memories` (admin tools, below) still distinguishes `MemoryCorruptionError` because `iter_all` is on its read path.
  - **Partial failure is failure.** When `report.errors` is non-empty the tool **raises** — even when some pairs saved and only others errored. The guard is `if report.errors:`. The client-facing message is counts-only: `RuntimeError("Memory import failed: N item(s) errored (X saved). A partial import is a failure; check the server logs.")`. The raw per-item exception text (which can carry the on-disk persist path / driver internals) is logged via `logger.error` and never returned to the client. Pinned by `tests/unit/test_memory_mcp_tool.py::test_tool_signals_error_on_partial_failure`.
- **Closure-bound store + single-writer lock.** The `MemoryStore` is constructed once at registration time and closed over by the tool, mirroring how the two core tools close over `cfg`. The `import_lock` (`asyncio.Lock`) was retained after issue #207 — not for dedup races (content-hash upsert is idempotent) but to keep predictable "last write wins" ordering when two concurrent imports target the same row. The bundle file format, content-hash id derivation, and storage shape are documented in [agent/memory.md](../agent/memory.md#first-party-importexport-srcsqllensmemory).
...
- **A single `admin_write_lock` (`asyncio.Lock`)** serializes the mutating tools (`delete_memory` / `clear_memories` / `add_memories`) so concurrent calls can't race each other's deletes or produce undefined "last write wins" ordering on a shared row. (Content-hash upsert in issue #207 means `add_memories` itself is now idempotent per row, but the lock still gates writes for predictable ordering.) The read-only tools (`list_memories` / `get_memory` / `export_memories` / `get_memory_stats`) do not take the lock.
...
| `add_memories` | `data_source_id`, `sql_pairs?`, `schema_docs?` | `{"saved_count", "errors": [...]}` | **Write-guarded.** Bulk add via `import_bundle` (idempotent content-hash upsert — re-adding the same logical record overwrites the existing row rather than duplicating it; the older `duplicate_count` / `skipped_count` response fields were dropped in issue #207). **Partial failure is failure**: any non-empty `errors` makes this an `isError` result (structured body still returned so the caller sees which rows failed). |
...
- **`add_memories` partial failure** → `_json_error(result)` carrying `errors[]`, after logging the failed/saved counts at `ERROR`.
...
### The self-driving memory-admin widget — `ui://sqllens/memory-admin.html`

`build_server` additionally registers a single `ui://` widget resource **inside** the same `if cfg.memory.allow_admin_tools:` block as the seven backing tools (issue #188). A host therefore never advertises a widget it cannot power: `resources/list` exposes `ui://sqllens/memory-admin.html` iff `allow_admin_tools` is set, with mime `text/html;profile=mcp-app` and `_meta.ui.prefersBorder = true`.

This is the second widget the server ships ([src/sqllens/ui/__init__.py](../../../src/sqllens/ui/__init__.py)). It reuses the same inliner — there is a second entry in `_RECIPES` keyed by `memory_admin.html` that applies the existing echarts + app-SDK splice functions. No new dependencies; no new build system.

Unlike `query_results.html` (a *push* widget — the model invokes `query_database` and the host pushes the `CallToolResult` into the widget via the App SDK's `ontoolresult` channel), `memory_admin.html` is **self-driving**: it pulls its own data on mount via `app.callServerTool(...)` and drives each user action by calling an admin tool directly. There is no launcher tool; hosts mount it via `resources/read`. No model is in the loop.

- **Topology-agnostic — no direct server reachability assumed.** The widget makes **no direct network request** to the server (or any external origin): no `fetch`, no `XMLHttpRequest`, no `WebSocket`, no `EventSource`, no `<script src>` to a server origin. Every server interaction is brokered by the host as a `tools/call` JSON-RPC over the App SDK's `postMessage` transport. So the widget renders identically whether a host reaches the server **directly** (public deployment) or **through a proxy** (private deployment). The fully-inlined, self-contained resource (vendored SDK + ECharts baked into one HTML payload) is a **hard requirement** for this — a widget that loaded `<script src>` from the server or fetched data directly would break under a private-network deployment. Pinned by `test_memory_admin_widget_makes_no_direct_network_calls` in `tests/unit/test_ui_widget.py`.
- **Mount-time wiring.** On `app.connect()` the widget invokes `Promise.all([loadBrowse(), loadStats()])`, which `callServerTool({name:"list_memories", arguments:{data_source_id, limit:1000}})` and `callServerTool({name:"get_memory_stats", arguments:{data_source_id}})` in parallel. The `data_source_id` argument is the placeholder `"memory-admin"` — the server's admin tools accept it as advisory (single-database server).
- **Four sections, each driving its own tool(s).** *Browse* — `list_memories` (windowed: capped at 1000, client-side filter and pagination, "showing N of M" banner) with a row-detail modal that calls `get_memory` and offers an inline window-confirmed `delete_memory`. *Import* — `add_memories` from pasted/uploaded `{sql_pairs, schema_docs}` JSON. *Stats* — `get_memory_stats` rendering count cards and an ECharts SVG bar chart of `top_hit_memories[]`. *Danger zone* — `export_memories` (JSON/CSV, same download cascade as `query_results.html`: `ui/download-file` → `clipboard.writeText` → `execCommand('copy')` → inline textarea overlay), `delete_memory` (window-confirm), and `clear_memories` (type-`CLEAR`-to-confirm + a second window-confirm). Each section's consumers read the server's wire shape directly — `memory_id` / `memory_type` / `timestamp` / `last_hit_date` are the canonical field names from `_record_to_wire` in [src/sqllens/memory/admin.py](../../../src/sqllens/memory/admin.py); the widget does no client-side reshaping.
- **`isError` contract — single seam.** Every tool call routes through one `callTool(name, args)` wrapper. It reads `result.isError` from the App SDK's `tools/call` response and renders a **loud red** banner (with the structured body or message). Per the CLAUDE.md `isError` contract, **partial `add_memories`** (server returns `isError=true` carrying `errors[]`) and **lossy `export_memories`** (server returns `isError=true` carrying `lossy:true`) paint red, never green. Destructive tools surface the server's authoritative `_require_write_auth` `RuntimeError` ("This tool mutates the memory store and is disabled on an unauthenticated endpoint…") through the same banner — the widget does not replicate auth logic.
- **Renderer.** ECharts SVG (`echarts.init(el, theme, {renderer:'svg'})`). The SVG renderer needs no `eval`/`Function` and emits plain DOM, so the widget runs inside a sandboxed iframe with a strict CSP (no `unsafe-eval` required) — matching the existing reasoning under [The MCP App interactive result widget](#the-mcp-app-interactive-result-widget).
- **Dev preview.** `scripts/memory_admin_preview.html` is the dev-only shim: a hand-rolled host stub that answers `ui/initialize` and routes `tools/call` to canned in-page responses (with toggles for forcing a lossy export and a partial-failure import). Open it in a browser to exercise every section without a live server.

### `_RECIPES` (the per-widget inlining table)

Both widgets share the same recipe (echarts first, then app SDK). Adding a new widget is the same shape: register an HTML file under `src/sqllens/ui/`, add a `_RECIPES` entry naming the bundles to splice in, and (only) inside the right gating block in `server.py` add a `@mcp.resource(...)` that returns `load_widget_html("your_widget.html")`.
...
|---|---|
| Successful query, dataframe / chart / prose result | Markdown serialization of the ordered blocks (tables, prose, chart placeholders, in stream order), `isError: false`. `_meta["sqllens/blocks"]` is attached when the blocks list is non-empty; the widget iterates each block and renders one container per block in stream order — no chart-vs-table precedence. The whole-array `_MAX_BLOCKS_TOTAL_BYTES` ceiling trims trailing blocks with an explicit notice. |
...
| Agent emitted an error status card, **not** superseded later in the turn | `AgentRunError("SQL execution error: " + <card description>)` (a `RuntimeError` subclass) → `isError: true`. Agent-authored detail passed through (categorized, not sanitized); logged server-side too. When `agent.show_details` is on the raised error carries the step-by-step trace, which `server.py` attaches to the `isError` result's `_meta["sqllens/agent_trace"]`; with it off the trace is `None` and the failure is formatted exactly as before. See [Agent trace channel](#agent-trace-channel--agentshow_details). |
| Agent emitted an error status card **then a later data `run_sql` succeeded** (self-correction) | The error is cleared; the recovered answer is returned as a normal `isError: false` result. The clear is scoped to a non-introspection data `run_sql` success and logged `INFO`. See [Agent self-correction supersedes an earlier error](#agent-self-correction-supersedes-an-earlier-error). |
