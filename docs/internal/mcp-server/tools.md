...
- **The DSL.** `EmitChartParams` (`src/sqllens/agent/tools/emit_chart.py`):
  ...
  - `title`: optional.
- **`components_to_chart` (in [tools/_format.py](../../../src/sqllens/tools/_format.py)).** Single-pass collapse, mirroring `components_to_table`: collect DataFrame components as Markdown tables, keep the last non-empty TEXT as the answer, and pick the **last** `CHART` component as `chart_payload`. A `STATUS_CARD` with `status='error'` short-circuits to `(error_message, True, None)`. Non-apps hosts still get the Markdown tables + answer. Note that the error card can come from *any* failed tool in the stream, not just `emit_chart` itself — issue #146 was a `save_question_tool_args` validation failure (the chart agent's memory-save omits `args`) that rendered an error card and short-circuited the chart here; see [agent/memory.md](../agent/memory.md#what-the-memory-feature-actually-does) for that fix.
