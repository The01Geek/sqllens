...
- **Schema introspection.** The guard is write-protection, not access control: a plain `SELECT` against a system catalog (`information_schema.*`, `sqlite_master`, `pg_catalog.*`) is a valid read, so it passes — there is no table allow/deny-list. `SHOW` / `DESCRIBE` / `PRAGMA` happen to be rejected only because they parse as non-`SELECT` roots, not because schema reads are blocked. SQL Lens is reached only through natural-language tool calls (the MCP client cannot send SQL directly; only the internal agent runs SQL), so the *soft* control against an end user prompting the agent into enumerating tables/columns is the **Data Confidentiality** directive in the default system prompt ([src/sqllens/agent/core/system_prompt/default.py](../../../src/sqllens/agent/core/system_prompt/default.py)). A prompt directive is bypassable; the authoritative boundary for keeping the schema confidential is a database role scoped to specific tables/views with no catalog access.
