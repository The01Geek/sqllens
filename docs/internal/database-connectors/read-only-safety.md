...
4. **Whitelist root expression types:** `Select`, `Union`, `Intersect`, `Except`, `With` (CTE chains). Anything else — `Insert`, `Update`, `Delete`, `Drop`, `Create`, `Alter`, `Pragma`, `Truncate`, etc. — is rejected by the negative-type-check at the root.
   - **Structural `SHOW` exception (schema discovery).** A single `exp.Show` root is also accepted, but *only* when its subkind is in the fail-closed allowlist `_SAFE_SHOW_SUBKINDS = {TABLES, COLUMNS, INDEX, CREATE TABLE, CREATE VIEW}`. This lets the agent discover schema on a fresh database (no ChromaDB memory yet) instead of failing before any query runs. Every other `SHOW` subkind is rejected — notably the info-leaking variants `SHOW GRANTS` (permissions), `SHOW PROCESSLIST` (cross-session SQL), `SHOW {MASTER,SLAVE,REPLICA} STATUS` (replication topology), `SHOW VARIABLES` / `SHOW STATUS` (server internals / secrets), and `SHOW DATABASES` / `SHOW SCHEMAS` (cross-schema disclosure: see [Why `SHOW DATABASES` is not allowlisted](#why-show-databases-is-not-allowlisted)). `SHOW` only parses to `exp.Show` under the **MySQL** dialect; every other dialect falls back to an opaque `exp.Command` rejected by the root-type check, so this exception is MySQL-only in practice. An accepted `SHOW` still passes through the full deny-walk in rules 5–7, so it cannot smuggle a side-effecting function (e.g. `SHOW COLUMNS FROM t WHERE Field = sleep(1)` is still rejected). `SHOW` is also classified read-shaped (`is_read_shaped`) so the runners route it through the row-returning branch rather than the rows-affected write branch.
...
7. **Reject side-effecting / DoS / RCE functions.** A syntactically valid `SELECT` can still call `load_extension` (SQLite RCE), `pg_read_file` (Postgres data exfiltration), or `SLEEP(60)` (MySQL DoS) — the root-type and DML/DDL walk do not see these. The guard maintains a per-dialect denylist (`_SIDE_EFFECT_FUNCS`) and a dialect-independent list (`_ALWAYS_DENIED_FUNCS`) and checks every `exp.Func` node in the walk against them. Both `exp.Anonymous` nodes (unknown functions, matched by written name) and typed `exp.Func` subclasses (known functions like `generate_series`, matched via `sql_names()`) are covered. For an unknown or `None` dialect the guard applies the union of every dialect's denylist — fail-closed.
  ...
  Issue #186 (2026-05-30 security audit) extended the Postgres entry with `pg_stat_file` / `pg_read_server_files` / `pg_ls_logdir` / `pg_ls_waldir` — built-ins (pg_stat_file from 9.x, pg_ls_logdir / pg_ls_waldir from 10, pg_read_server_files from 11) that disclose the on-disk WAL/log layout to anyone who can issue a `SELECT`; they parse as `exp.Anonymous` and would otherwise slip the guard. The MySQL entry gained `sys_exec` / `sys_eval`, the `lib_mysqludf_sys` UDFs: when the UDF is installed and the configured DB role has `EXECUTE`, they are an RCE seam, so they need explicit naming to stay refused. Both additions are pinned by `tests/unit/test_safety.py::TestCorpusStaysRejected::test_corpus_entry_still_rejected` (they are entries in `_BYPASS_CORPUS`; the narrower `TestSideEffectFunctionsRejected` parametrize list does not enumerate them).
...
### Why `SHOW DATABASES` is not allowlisted

`SHOW DATABASES` (and its sqlglot-normalised spelling `SHOW SCHEMAS`) was removed from `_SAFE_SHOW_SUBKINDS` in issue #218. The command returns **every schema visible to the configured MySQL role**, not just the one the operator named in `database.url`. CLAUDE.md asserts the contract is *"one database per running instance"* — allowlisting `SHOW DATABASES` would let a client (and through prompt-injection, an attacker via row content) enumerate sibling schemas the DSN never named, which is exactly the cross-trust-boundary disclosure the read-only guard otherwise plugs.

The other allowlisted subkinds (`TABLES` / `COLUMNS` / `INDEX` / `CREATE TABLE` / `CREATE VIEW`) are naturally scoped to the connected schema (or take an explicit `FROM <table>` argument that the connector's session-level schema selection bounds), so they stay aligned with the one-database-per-instance contract.

The agent still has two non-disclosure-widening paths to learn the active schema:

- A plain `SELECT` over `information_schema.tables` / `information_schema.columns` — `information_schema` is in `_CATALOG_SCHEMAS` so the result is classified as introspection by `is_introspection_query` and never surfaced as a recovered answer.
- The per-runner `list_data_sources` tool, which describes the configured database without consulting cross-schema metadata.

Pinned by `tests/unit/test_safety.py::TestUnsafeShowRejected::test_unsafe_show_rejected[SHOW DATABASES]` / `[SHOW SCHEMAS]`.
...
## Schema-introspection classifier (`is_introspection_query`)

`is_introspection_query(sql, *, dialect=None)` in [src/sqllens/safety/readonly.py](../../../src/sqllens/safety/readonly.py) (exported from `sqllens.safety`) is **not a guard** — it neither blocks nor rewrites SQL. It is a read-only *classifier* that answers one question: *is this SQL a schema-introspection read, or a query that answers the user's data question?* It lives alongside the guard because it shares the same `sqlglot` parse and the same notion of "catalog read" the guard's `What the guard does not check` section describes, but its only consumer is the MCP formatter (`tools/_format.py`).

It exists for the agent self-correction case: the system prompt lets the agent run a catalog `SELECT` or a structural `SHOW` internally to confirm a column/table name after a failed query, then re-issue a corrected query. When the MCP formatter collapses the component stream it must distinguish "the agent looked up the schema" (a step toward an answer) from "the agent re-ran the actual query and got data" (the recovered answer that supersedes an earlier error). Counting an introspection success as the recovered answer would let *data query failed → introspect → give up* report a silent success — exactly the failure CLAUDE.md forbids. See [mcp-server/tools.md](../mcp-server/tools.md#agent-self-correction-supersedes-an-earlier-error) for the consuming logic.

Classification rules (returns `True` for introspection):

- A statement whose leading keyword is `SHOW` (the structural schema-discovery `SHOW` variants the guard allowlists — `SHOW TABLES`/`COLUMNS`/...).
- A `SELECT` referencing any table whose schema is a catalog: `information_schema`, `pg_catalog`, `mysql`, `sys`, or `performance_schema` (`_CATALOG_SCHEMAS`).
- A `SELECT` referencing any table whose **name** begins with a catalog object prefix: `pg_` (Postgres system tables like `pg_class`) or `sqlite_` (`sqlite_master`, `sqlite_schema`) (`_CATALOG_NAME_PREFIXES`).

Everything else — including a normal `SELECT` over user tables — classifies as a data query (`False`).

**Fail-open to `False`.** Unlike the guard (which fails *closed* to `UnsafeSqlError`), this classifier returns `False` on a parse failure or `None` parse result. The bias is deliberate and opposite the guard's: a string reaching this function has *already executed* through `run_sql`, so a misparse here is rare; treating an unparseable string as a data query biases toward **surfacing a recovered answer** rather than swallowing a real one as "just introspection". Misclassification is a UX concern (a recovered answer shown or hidden), not a safety one — the guard already vetted what was allowed to run.

Pinned by `tests/unit/test_safety.py`.
