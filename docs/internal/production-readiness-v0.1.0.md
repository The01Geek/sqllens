...
| ~~O-18~~ | `.github/workflows/claude-runner.yml` | **Resolved.** `claude-code-action` attempted an OIDC → app-token exchange that 401s when the job runs inside a `workflow_call` from `pull_request_target` (upstream issues anthropics/claude-code-action#443, #721). Fixed by passing `github_token: ${{ secrets.GITHUB_TOKEN }}` explicitly, which makes the action skip the exchange. Side-effect: progress comments and final review post under `github-actions[bot]` instead of `@claude`. | — |
