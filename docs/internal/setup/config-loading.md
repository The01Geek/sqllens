...
|---|---|---|
| `config_version` | — | Top-level scalar (not a table), defaults to `1`. **Accepted-but-ignored** — declared so `extra = "forbid"` doesn't reject it; reserved so a future schema migration can branch on it. No behaviour keys off it yet. |
| `[database]` | `url` | `name` defaults to `"primary"`. `read_only` defaults to `true` (enforced by the SQL parser guard, not the SQLite driver). `statement_timeout_ms` defaults to `30_000` (30s; `0` disables on Postgres/MySQL). `max_rows` defaults to `10_000`, bounded `1..1_000_000`. Both are applied per-runner via the engine's native primitive and surface a truncation hint to the LLM — see [database-connectors/read-only-safety.md](../database-connectors/read-only-safety.md). Exposes a read-only `dialect` property (not a config field): the SQLAlchemy URL scheme with any `+driver` suffix stripped (`sqlite:///demo.db` → `sqlite`, `mysql+pymysql://…` → `mysql`, `postgresql+psycopg://…` → `postgresql`). |
...
| `[server]` | — | `transport` defaults to `"stdio"`. `host`/`port` only used for `transport = "http"`. `log_level` defaults to `"info"`, domain `critical \| error \| warning \| info \| debug \| trace` (the `trace` level is uvicorn's; this domain is deliberately the wider one). **Field + schema validation only — no uvicorn/logging wiring yet** (lands in a later issue). |
| `[agent]` | — | `max_tool_iterations` defaults to `20`. Raised from the framework's built-in `10` — real-world schema exploration requires more iterations. Env var: `SQLLENS_AGENT__MAX_TOOL_ITERATIONS`. `show_details` defaults to `true` (env var `SQLLENS_AGENT__SHOW_DETAILS`). When on, `query_database` surfaces the executed SQL alongside the answer: as a fenced ```sql block in the Markdown text *and* as structured data under `_meta["sqllens/query"]` for apps-aware hosts. Set to `false` to restore the pre-feature answer-only behavior byte-for-byte. The toggle is consumed by the agent factory (it unlocks `UI_FEATURE_SHOW_TOOL_ARGUMENTS` for the static default user group only) — see [agent/factory.md](../agent/factory.md#show_details--ui-feature-unlock) — and by `tools/_format.py` + `server.py`, see [mcp-server/tools.md](../mcp-server/tools.md#executed-sql-channel--agentshow_details). |
| `[agent.audit]` | — | Nested `AuditConfig` (defaulted). Defines the future audit-logging surface; **the agent factory does not consume it yet**. `enabled` defaults to `false` (master gate — when off the other three fields are inert and no audit logger is built). `log_level` defaults to `"info"`, domain `critical \| error \| warning \| info \| debug` (note: **no `trace`** — narrower than `server.log_level`; the two map to different sinks and must not be conflated by the future wiring). `include_response_text` defaults to `false` — maps to the `include_full_text` per-call argument of `AuditLogger.log_ai_response`. `sanitize_parameters` defaults to `true` — maps to the `sanitize_parameters` per-call argument of `AuditLogger.log_tool_invocation`. (These are per-call arguments, not constructor parameters on `LoggingAuditLogger`.) This model sets its own `extra = "forbid"` (see below). |
| `[[rls]]` | — | Opt-in array of `RlsRule` row-level-security predicates injected into every agent-generated read of `table`. Defaults to empty (the `RlsGuardRunner` decorator is then not wrapped at all). Each rule sets `table`, `column`, `operator` (one of `= != < <= > >= in`), and **exactly one** of `value` (static — works on every transport) or `value_from_metadata` (dynamic — key resolved per request from caller-supplied MCP `_meta`, HTTP transport only). Identifier fields go through a strict allowlist (`^[A-Za-z_][A-Za-z0-9_]*$`); the operator is normalized to lower-case. The model sets `extra = "forbid"` *and* `frozen = True` — a misspelled key inside a `[[rls]]` block fails loudly at config load rather than silently dropping the predicate (a dropped RLS predicate is an unfiltered query, the exact failure the feature exists to prevent), and the validated invariants must hold for the rule's lifetime. Full reference in [database-connectors/row-level-security.md](../database-connectors/row-level-security.md). |

`extra = "forbid"` is set on the top-level `Config`, so unknown **top-level** keys raise a `ValidationError` rather than being silently dropped. The top-level setting does **not** cascade into nested tables, so a misspelled key inside a sub-section is normally accepted-and-dropped. `AuditConfig` overrides this with its own `model_config = ConfigDict(extra="forbid")`: a typo inside `[agent.audit]` (e.g. `sanitize_paramters`) fails loudly at load — via TOML *or* the `SQLLENS_AGENT__AUDIT__` env path — rather than silently reverting to the privacy-safe default, because a silent revert on a privacy surface is the worst outcome.
...
**Single path resolution (C-5).** `Config.load()` resolves the TOML path **exactly once** — via `_resolved_toml_path()` called *after* the `SQLLENS_CONFIG` mutation and *before* `cls()` — and stashes the result in the `_LOAD_TOML_PATH` `ContextVar`. `settings_customise_sources` reads that stashed `Path` instead of re-resolving, and the BOM re-read in the `except` branch reuses the same value rather than calling `_resolved_toml_path()` a second time. This closes a TOCTOU window where a config file could be created/removed/swapped between the parse-source resolution and the BOM check, making the two see different files. A `ContextVar` (not a module global) keeps concurrent in-process loads from clobbering each other's resolved path; it is `reset()` in the `finally` block. Direct `Config()` construction outside any `Config.load()` (tests/programmatic) finds the var unset (`LookupError`) and resolves the path fresh in `settings_customise_sources` — that is the expected path, not an error.
...
The top-level `extra = "forbid"` does **not** propagate into nested sub-models. For most sub-sections an accepted-and-dropped typo is tolerable, but for a privacy/audit surface a silent revert to the safe default is the worst outcome — set `model_config = ConfigDict(extra="forbid")` on that sub-model so a misspelled key fails loudly via both the TOML and `SQLLENS_<SECTION>__<FIELD>` env paths. `AuditConfig` is the reference example.
