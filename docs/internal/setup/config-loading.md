...
Success output (`Wrote <path>`, `Config OK` + summary lines, `sqllens version`) is left on stdout, because by the time those print FastMCP has not yet taken over stdout (commands other than `serve`) or the CLI is exiting with a clean status without ever calling `mcp.run()`.
