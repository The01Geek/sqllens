...
`serve` and `validate` route the config-load exception through `_format_config_error` (in [cli.py](../../../src/sqllens/cli.py)) before printing. For a pydantic `ValidationError` it renders only each error's `loc`/`msg`/`type` (as `<loc>: <msg> [<type>]`) and **drops `input`/`ctx`**, so a schema-validation failure no longer echoes the offending input — bearer token, LLM API key, or DSN password (the latter is a plain `str` field, not a self-masking `SecretStr`) — to stderr. Non-`ValidationError` config-load errors (file-not-found, BOM, TOML syntax) pass through unchanged because their messages are the actionable remediation and don't embed schema input; a `tomllib` *syntax* error on a secret-bearing line can still echo that line verbatim, an accepted residual since config files are operator-readable by design.
