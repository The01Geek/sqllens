...
3. `"(unknown)"` — final fallback if neither source returns a value, or if the `gh` CLI is not installed.

This fallback chain means the script degrades gracefully in Actions environments where `GITHUB_TOKEN` is a fine-grained installation token that returns HTTP 403 on `GET /user`.
