...
class DefaultSystemPromptBuilder(SystemPromptBuilder):
    ...
    async def build_system_prompt(
        ...
        prompt_parts = [
            ...
            "- When a query returns a single row, state the answer in a plain sentence in your response (for example, \"There were 1,234 orders last month.\").",
            "",
            "Tool Errors:",
        ...
        if has_emit_text:
            prompt_parts.extend(
                [
                    ...
                    "=" * 60,
                    "",
                    "Your answer is an *ordered sequence of blocks* — query results (from run_sql: a table for multi-row results, short text for a one-row result), charts (from emit_chart), and prose (from emit_text) — rendered to the user in the order you produce them. The user SEES the blocks; everything else (your reasoning, tool-call narration, mid-loop thoughts) is hidden.",
                    "",
                    "RULES:",
