...
class DefaultSystemPromptBuilder(SystemPromptBuilder):
    ...
    async def build_system_prompt(
        ...
        prompt_parts = [
            ...
            "- When a query returns more than one row, or a single row with more than four columns, the result is shown to the user as a table outside of your response, so do NOT repeat the table in your response. Focus on summarizing and interpreting the results.",
            "- When a query returns a single row with four or fewer columns that fits the display size limit, it is shown as short text rather than a table, so state the answer in a plain sentence in your response (for example, \"There were 1,234 orders last month.\").",
            "- If a single row is too large to display, the user does not see its values, so state its key values in your response.",
            "",
            "Tool Errors:",
            ...
            "- Schema-introspection queries ARE allowed when you need them to write a correct query — for example, to confirm a column name before retrying after an 'Unknown column' error, or when memory contains no relevant schema for the table you are querying. Express introspection as a SELECT against the catalog (e.g. information_schema.columns, sqlite_master, or pg_catalog); on MySQL the structural SHOW variants (SHOW TABLES, SHOW COLUMNS, SHOW DATABASES, SHOW INDEX, SHOW CREATE TABLE/VIEW) are also permitted. DESCRIBE and the information-disclosing SHOW variants (SHOW GRANTS, SHOW PROCESSLIST, SHOW VARIABLES, SHOW STATUS, replication-status) are not permitted. Run them internally to inform your own SQL; their results must NOT be summarized, listed, output as DDL, or otherwise echoed back to the user.",
        ...
        if has_emit_text:
            prompt_parts.extend(
                [
                    ...
                    "=" * 60,
                    "",
                    "Your answer is an *ordered sequence of blocks* — query results (from run_sql: short text for a one-row result with four or fewer columns that fits the display size limit, otherwise a table), charts (from emit_chart), and prose (from emit_text) — rendered to the user in the order you produce them. The user SEES the blocks; everything else (your reasoning, tool-call narration, mid-loop thoughts) is hidden.",
                    "",
                    "RULES:",
                    ...
                    "  • Do NOT call emit_text just to say \"here is your data\" — let a multi-row table or a chart speak for itself. emit_text earns its place when it adds context the artifact cannot convey. For a one-row result shown as short text, DO use emit_text to state the answer in a plain sentence.",
