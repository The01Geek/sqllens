...
class DefaultSystemPromptBuilder(SystemPromptBuilder):
    ...
    async def build_system_prompt(
        ...
        prompt_parts = [
            ...
            "- When a query returns more than one row, or a single row with many columns, the result is shown to the user as a table outside of your response, so do NOT repeat the table in your response. Focus on summarizing and interpreting the results.",
            "- When a query returns a single row with only a few columns, it is shown as short text rather than a table, so state the answer in a plain sentence in your response (for example, \"There were 1,234 orders last month.\").",
            "",
            "Tool Errors:",
