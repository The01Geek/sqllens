...
class DefaultWorkflowHandler(WorkflowHandler):
    ...
    def _generate_user_starter_card(self, analysis: Dict[str, Any]) -> UiComponent:
        ...
        if not analysis["has_sql"]:
            content = (
                "# ⚠️ Setup Required\n\n"
                "SQL Lens requires configuration before it can help you analyze data."
            )
        else:
            content = (
                "# 👋 Welcome to SQL Lens\n\n"
    ...
    def _generate_setup_guidance(
        ...
        if not analysis["has_sql"]:
            ...
            content = (
                "## 🚨 Setup Required\n\n"
                "To get started with SQL Lens, you need to configure a SQL connection tool:\n\n"
                "```python\n"
            ...
            if not analysis["has_memory"]:
                suggestions.append(
                    ...
                    "```python\n"
                    "from sqllens.agent.tools import SearchSavedCorrectToolUses, SaveQuestionToolArgs\n"
            ...
            if not analysis["has_viz"]:
                suggestions.append(
                    ...
                    "```python\n"
                    "from sqllens.agent.tools import VisualizeDataTool\n"
    ...
    async def _generate_status_check(
        ...
        if analysis["is_complete"]:
            status_content += (
                "🎉 **Excellent!** Your SQL Lens setup is complete and optimized.\n\n"
            )
