"""
Agent implementation for the SQL Lens agent.
...
from sqllens.agent.core.storage import ConversationStore
from sqllens.agent.core.llm import LlmService
from sqllens.agent.core.system_prompt import SystemPromptBuilder
from sqllens.agent.core.storage import Conversation, Message
from sqllens.agent.core.llm import LlmMessage, LlmRequest, LlmResponse
from sqllens.agent.core.tool import ToolCall, ToolContext, ToolResult, ToolSchema
from sqllens.agent.core.user import User
from sqllens.agent.core.registry import ToolRegistry
from sqllens.agent.core.system_prompt import DefaultSystemPromptBuilder
from sqllens.agent.core.lifecycle import LifecycleHook
from sqllens.agent.core.middleware import LlmMiddleware
from sqllens.agent.core.workflow import WorkflowHandler, DefaultWorkflowHandler
from sqllens.agent.core.recovery import ErrorRecoveryStrategy, RecoveryActionType
from sqllens.agent.core.enricher import ToolContextEnricher
...
from sqllens.agent.core.audit import AuditLogger
...
class Agent:
    ...
    def __init__(
        ...
        if conversation_store is None:
            from sqllens.agent.integrations.local import MemoryConversationStore
        ...
        while tool_iterations < _max_iterations:
            ...
            if response.is_tool_call():
                ...
                if response.content:
                    ...
                    # Marker (sqllens.agent.markers.IS_ANSWER_MARKER_KEY) shared
                    # with EmitTextTool: the MCP-layer block builder reads it to
                    # include the terminal answer as a rendered TEXT block while
                    # excluding the intermediate-reasoning TEXT yielded above
                    # (gated on UI_FEATURE_SHOW_TOOL_INVOCATION_MESSAGE_IN_CHAT).
                    yield UiComponent(
                        rich_component=RichTextComponent(
