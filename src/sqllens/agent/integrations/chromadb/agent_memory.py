...
from sqllens.agent.capabilities.agent_memory import (
    AgentMemory,
    TextMemory,
...
class ChromaAgentMemory(AgentMemory):
    ...
    Example:
        >>> from sqllens.agent.integrations.chromadb import ChromaAgentMemory
        ...
        def _search():
            ...
            if results["ids"] and len(results["ids"][0]) > 0:
                for i, (id_, distance, metadata) in enumerate(
                    ...
                    # ChromaDB's default 'l2' metric returns squared L2: d = Σ(Ai-Bi)².
                    # For normalized embeddings: d = 2(1 - cos_sim), so cos_sim = 1 - d/2.
        ...
        def _search():
            ...
            if results["ids"] and len(results["ids"][0]) > 0:
                for i, (id_, distance, metadata) in enumerate(
                    ...
                    # Squared L2 → cosine similarity (see search_similar_usage)
                    similarity_score = max(0, 1 - distance / 2)
