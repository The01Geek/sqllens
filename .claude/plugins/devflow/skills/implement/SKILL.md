...
1. **Now-orphaned public surfaces.** For every public method or function you removed the *callers* of (not the function itself), and for every file/asset the just-deleted code was the sole consumer of: grep the whole repo for remaining references. Zero references → it is part of *this* removal; delete it too. (E.g. `OrderService::saveShipment` left as a zombie public method with zero callers after the removal; `webshiptestlabel.png` left after `label.php` was deleted.)
2. **Dead arguments to changed callees.** For every callee whose signature or body you changed so it stops reading some inputs: re-check each call site and stop passing the now-ignored arguments/keys. (E.g. `CheckoutAjax::handleLoadShippingOptions` kept passing 5 now-dead keys into `handleLoadShippingOptionQuote` after the receiver stopped reading them.)
3. **Surviving inbound links and route refs.** For every page, route, endpoint, or file path you deleted: grep the repo for that path/URL/route name (links in templates, menu/nav configs, `href`s, redirects, `Plugin_*` route tables, sitemap entries). Every surviving reference is a regression — remove the link, or restore the target if it was deleted in error. (E.g. `sellers-portal/reports/all.php` still linked to `/sellers-portal/reports/withdrawal` after `reports/customer/withdrawal.php` was deleted → sellers hit a 404.)
4. **In-scope subtree completeness.** If the issue scopes a directory/feature subtree for removal, walk the *whole* subtree — do not stop at the files the obvious entry points reference. An untraversed leaf page that still calls the deleted integration is in scope by definition. (E.g. an orphan `sellers-portal/reports/sales/comissions.php` left in place still calling `Plugin_Reports`, linked from `reports/all.php`, despite sitting inside the in-scope commission subtree.)
...
Same principle as 2.3.1, applied to `CLAUDE.md` conventions instead of dead code: **any function, method, SQL string, or new file your diff added or modified lines in must conform to the conventions in `CLAUDE.md` when you leave it** — even if the violation was already there before you touched it, and even if "everything around it does it the same way." Recurring offenders that reviewers keep flagging as *Important* and that then ship anyway:

- A PHP method left with multiple positional parameters after you edited its signature (e.g. dropped one argument) — CLAUDE.md PHP *Function Parameters* mandates a single `$params` array.
- A raw SQL string in code you touched missing backticks around table/column names, or lowercasing MySQL keywords — CLAUDE.md *SQL Style Guidelines*.
- A new variable, method, file, or identifier you introduced that copies a legacy misspelling or non-`lowerCamelCase` name from a sibling file — CLAUDE.md *Naming Conventions*. "It matches the established convention across the existing tools" is **not** a valid reason to propagate a misspelled or non-conforming name into new code; name the new thing correctly.

After implementing, before running tests, do this sweep:

1. From `git diff --staged -U0` (or `git diff -U0`), list every function/method/SQL string/new file your diff added or changed lines in.
2. Re-read each one in its post-edit state and check it against `CLAUDE.md` (Naming Conventions, PHP Function Parameters, SQL Style Guidelines, If/Else formatting, Multi-Condition Expressions, the React/JS rules — whichever apply).
